...
class EMA(nn.Module):
    ...
    def forward(self, x):
        # x: [Batch, Time, Channel], output均为: [Batch, Time, Channel]，channel指的是，Time指的是序列长度，Batch指的是批量大小/数据量
        _, t, _ = x.shape
        # 1. 计算权重
        powers = torch.flip(torch.arange(t, dtype=torch.double), dims=(0,))
        weights = torch.pow((1 - self.alpha), powers).to(self.device)
        # .to('cuda')
        ...
        x_trend = torch.cumsum(x * weights, dim=1)
        # 4. 归一化
        x_trend = torch.div(x_trend, divisor)
        x_seasonal = x - x_trend
        return x_trend.to(torch.float32), x_seasonal.to(torch.float32)
...
class PINNsformer(nn.Module):
    ...
    def forward(self, x, t): #输入的x和t形状一般为(点的数量, 序列长度, 1)
        ...
        return output #输出形状仍然是(点的数量，序列长度，1)
    
class ShuangliuPINNsformer(nn.Module):
    def __init__(self, d_out, d_model, d_hidden, N, heads, device, alpha=0.1):
        super(ShuangliuPINNsformer, self).__init__()
        self.pinnsformer_1 = PINNsformer(d_out, d_model, d_hidden, N, heads)
        self.pinnsformer_2 = PINNsformer(d_out, d_model, d_hidden, N, heads)
        self.device = device
        self.ema = EMA(alpha,self.device)
    ...
    def forward(self, x, t):
        # src = torch.cat((x,t), dim=-1) #将x和t沿最后一个维度拼接，形成输入的特征向量，形状变为(点的数量，序列长度，2)
        x_trend, x_seasonal = self.ema(x)
        t_trend, t_seasonal = self.ema(t)
        output_trend = self.pinnsformer_1(x_trend, t_trend)
        output_seasonal = self.pinnsformer_2(x_seasonal, t_seasonal)
        output = torch.cat((output_trend, output_seasonal), dim=-1)#形状为(点的数量，序列长度，2)
