...
class AddMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        attn_weights = attn_weights[:, :, 0] + lambda_full * attn_weights[:, :, 1] + lambda_full**2 * attn_weights[:, :, 2] + lambda_full**3 * attn_weights[:,:,3] #最终的权重形状变为(batch_size, h_head, 查询的序列长度, 键值的序列长度)
